...
export const repos = sqliteTable(
  'repos',
  {
    ...
    // Presence/lifecycle only. "Archived" is NOT here — it's the `archivedAt` flag below,
    // an orthogonal axis Archive/Restore toggle without touching `status`.
    status: text('status', {
      enum: ['active', 'missing', 'purged'],
    })
      .notNull()
      .default('active'),
    ...
    // Serve-block flag (set = "archived"), orthogonal to status. Anchors grace deadlines.
...
export type RepoStatus = 'active' | 'missing' | 'purged';
