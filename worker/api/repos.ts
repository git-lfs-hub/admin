...
const app = new Hono<AppEnv>()
  ...
  // Archive: block serving on lfs-server, then mark `archived`. Inline, no workflow.
  // RPC runs before the status flip — a failed block leaves the row unchanged (missing).
  .post('/:owner/:repo/archive', async (c) => {
    ...
    const row = await repos.markArchived(owner, repo);
    if (!row) return c.json({ error: 'invalid_state' }, 409);
    return c.json({ repo: row });
  })
  // Restore (undo Archive): unblock serving, then mark `active`. Inline — live storage
  // is present the whole archived window without cold storage. Unblock runs before the
  // flip, so a failed RPC leaves the row `archived`. Cold restore (clearedAt set →
  // Glacier retrieval) lands in Group D3.
  .post('/:owner/:repo/restore', async (c) => {
    const { owner, repo } = c.req.param();
    const repos = c.env.REPOS.getByName('global');
    const cur = await repos.get(owner, repo);
    if (!cur) return c.json({ error: 'not_found' }, 404);
    if (cur.status !== 'archived') return c.json({ error: 'invalid_state', status: cur.status }, 409);
    try {
    ...
    const row = await repos.markActive(owner, repo);
    if (!row) return c.json({ error: 'invalid_state' }, 409);
    return c.json({ repo: row });
  })
